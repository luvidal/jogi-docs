...
async function extractFace(imageBuffer, _mimetype, _model, opts) {
  const log = getLogger();
  const oriented = await sharp2(imageBuffer).rotate().toBuffer();
  ...
  try {
    ...
    const extended = needsExtend ? await sharp2(oriented).extend({ top: extT, bottom: extB, left: extL, right: extR, background: "#FFFFFF" }).toBuffer() : oriented;
    const photo = await sharp2(extended).extract({ left: cropLeft, top: cropTop, width: cropW, height: cropH }).resize(256, 256).jpeg({ quality: 92 }).toBuffer();
...
function getPromptVersion() {
  return createHash("sha256").update(JSON.stringify(getDoctypes())).update(PROMPT_TEMPLATE_VERSION).update(JSON.stringify(getSchemaVersionPayload())).digest("hex").slice(0, 12);
}
...
export { Doc2Fields, buildCacheKey, buildClassifyResponseSchema, buildDataSchemaForDoctype, buildExtractResponseSchema, buildShapeOnlyClassifyResponseSchema, configure, detectAndSplitCompositeCedula, detectAndSplitCompositeCedulaV3, detectCedulaSide, extractFace, extractPdfPageAsImage, getPromptVersion, mergeCedulaFiles, queryGrounded };
